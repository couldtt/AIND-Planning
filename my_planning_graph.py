...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if pg_a.prenodes.issubset(self.s_levels[level]):
                ...
                for prenode in pg_a.prenodes:
                    prenode.children.add(pg_a)
                    pg_a.parents.add(prenode)
                    self.s_levels[level].add(prenode)
    ...
    def add_literal_level(self, level):
        ...
        for pg_a in self.a_levels[level - 1]:
            for effnode in pg_a.effnodes:
                pg_s_new = effnode
                pg_s_new.parents.add(pg_a)
                pg_a.children.add(pg_s_new)
                self.s_levels[level].add(pg_s_new)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        return len(set(node_a1.action.effect_add).intersection(set(node_a2.action.effect_rem))) > 0 or \
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        return any(fluent in node_a2.action.precond_pos for fluent in node_a1.action.effect_add) or \
               any(fluent in node_a1.action.precond_pos for fluent in node_a2.action.effect_add)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        return any(parent1.is_mutex(parent2) or parent2.is_mutex(parent1)
    ...
    def h_levelsum(self) -> int:
        ...
        for num, goal in enumerate(self.problem.goal):
            found = False
